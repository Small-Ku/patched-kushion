...
The `Update` workflow resolves the patch-supported app version and separates architecture policy from concrete artifacts. Patched apps without an explicit architecture use the auto policy: probe a real `universal` output plus all four supported ABI splits, then publish every variant that current stock sources can produce. Explicit `arches` remain hard requirements. Stock acquisition is multi-source: package-derived Aptoide and APKPure/apkeep fallbacks are tried before fragile Archive/APKMirror fallbacks, and every downloaded APK or split must still match the pinned upstream signer. Third-party stores are treated as untrusted transport: normalized stock also receives a package/version/permission/component/DEX/native security fingerprint, configured IoCs are quarantined, and an independently available store/mirror is used opportunistically to corroborate store-sourced stock. A missing corroborator does not turn an otherwise valid pinned artifact into an availability failure, while an actual fingerprint disagreement is quarantined.
For split-distributed stock apps, APKM/APKS/XAPK is preferred when it can cover the requested outputs. The shared-source stage prefers direct input and APKPure/apkeep before Uptodown/Archive, with APKMirror release inventory retained as the last shared fallback; when reached, APKMirror bundle variants are ranked by requested-ABI coverage, intrinsic ABI breadth, minimum-Android compatibility, and density breadth. The selected broad bundle is downloaded once and partitioned once into common splits plus ABI buckets. ABI-specific branches keep the base APK, requested ABI split, and every non-ABI split before APKEditor merges the set; no explicit DPI setting means density-range bundles remain eligible. The universal branch keeps the complete coherent split set. This preserves language, density, and feature coverage while ABI-specific outputs remove only foreign CPU ABIs.
The Update workflow therefore fans out in three levels: one shared source branch per app/version, architecture merge branches from that partition, then pending APK/module patch branches from each normalized stock artifact. Apps whose sources expose no reusable split container fall back to per-ABI acquisition.
