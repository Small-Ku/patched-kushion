...
unchanged=$(PATH="$tmp/bin:$PATH" FAKE_RELEASES="$tmp/releases.json" \
...
changed=$(PATH="$tmp/bin:$PATH" FAKE_RELEASES="$tmp/releases-new.json" \
  GITHUB_REPOSITORY=example/patched-kushion \
  python3 "$root/scripts/fdroid_sources.py" probe \
    --config "$tmp/sources.toml" --provenance "$tmp/provenance.json")
...
missing=$(PATH="$tmp/bin:$PATH" FAKE_RELEASES="$tmp/releases.json" \
  GITHUB_REPOSITORY=example/patched-kushion \
  python3 "$root/scripts/fdroid_sources.py" probe \
