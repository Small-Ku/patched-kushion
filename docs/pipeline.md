...
A partial broad result is useful rather than terminal. Source preserves the branches it can derive (for example an APKPure fat APK as `universal`) and continues the DAG only for the missing desired/optional architectures. Partitioned broad containers are materialized into self-contained branches before results from another provider are mixed in, producing one hybrid branch plan with per-branch provenance. Within each missing architecture, Source evaluates every successful provider candidate instead of stopping at the first runnable APK: a validated split container outranks a flattened standalone, and smaller candidates win within the same representation class. APKFab exact-version XAPKs are one such branch candidate and remain subject to the same pinned-signer, package/version, split-shape, and security gates as every other third-party transport. This lets a later APKM/APKS/XAPK source replace a much larger store-flattened APK without sacrificing language/density/feature splits.
...
- Successful Patch handoffs are cached by version-specific `inputId`, exact prepared-stock SHA-256, and the SHA-256 of the configured PKCS#12 signing identity. This conservative identity is computable before the locked JVM is restored, so a Patch-result cache hit can bypass the large Pixi/toolchain cache entirely. A new patch bundle/profile changes `inputId`; changed stock bytes change the stock digest; rotating or re-encoding the signing identity changes the signing digest. Any of those naturally misses the cache. Manual `force_build=true` bypasses the Patch-result cache but may still reuse already verified Source/Stock inputs. Final Package still runs for a new generation, which is important because module `versionCode` follows the numeric Release tag.
- Stage caches are restored before heavyweight toolchain caches whenever their identity can be computed from checkout metadata and small handoff inputs. Source, Stock, and Patch cache hits therefore validate and hand off without restoring the ~hundreds-of-megabytes Pixi environment; Package also delays toolchain setup until after reuse/patch-status gates. Cache misses still enter the same setup-pixi project cache. `pixi.lock` pins Python, GraalVM 25, `fdroidserver`, and transitive packages. Android Build Tools stay official SDK components, but their version and bounded `sdkmanager` lifecycle are exposed through the Pixi `android-tools` task.

Private signing material is never cached; only its SHA-256 identity participates in a Patch cache key. GitHub Release assets remain the publication/state boundary rather than being treated as an Actions cache. Pixi cache reuse restores the JVM distribution and environment, not GraalJIT-compiled machine code from a previous runner process.
